...
//     if (existing.user_id !== userId) throw new ForbiddenException('Kh�ng c� quy?n');
...
//     if (!existing) throw new NotFoundException('Kh�ng t�m th?y daily report');
//     if (existing.user_id !== userId) throw new ForbiddenException('Kh�ng c� quy?n');
//     if (existing.status !== ApprovalStatus.PENDING) throw new BadRequestException('Ch? du?c x�a khi PENDING');
...
//     if (report.status !== ApprovalStatus.PENDING) throw new BadRequestException('Daily report kh�ng ? tr?ng th�i PENDING');
